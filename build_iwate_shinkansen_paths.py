# -*- coding: utf-8 -*-
"""
岩手県マップ(iwate_map_tool_old.html)に「新幹線」レイヤーを追加するための
一回限りの変換スクリプト。

入力:
  - _gis_source_extra/rail/extracted/N02-20_GML/N02-20_RailroadSection.geojson
    (国土数値情報 鉄道データ、全国、令和2年度、商用可)

出力:
  - _gis_source_extra/shinkansen_path.json
"""
import json
import math
import os
...
SRC = os.path.join(BASE, "_gis_source_extra", "rail", "extracted", "N02-20_GML", "N02-20_RailroadSection.geojson")
...
def build_shinkansen():
    with open(SRC, encoding="utf-8") as f:
        data = json.load(f)
    pieces = []
    for feat in data["features"]:
        if feat["properties"].get("N02_002") != "1":
            continue
        geom = feat["geometry"]
        lines = geom["coordinates"] if geom["type"] == "MultiLineString" else [geom["coordinates"]]
        for line in lines:
            pieces.extend(line_to_path_pieces(line))
    return " ".join(pieces)


def main():
    d = build_shinkansen()
    print(f"shinkansen: {len(d)} chars")
    out_path = os.path.join(OUT_DIR, "shinkansen_path.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"shinkansen": d}, f)
